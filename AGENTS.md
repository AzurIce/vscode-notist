...
- 构建链：tsc 只做类型检查（`-noEmit`），产物由 esbuild 打包（CJS，external: vscode）；`out/` 与 `node_modules/` 不进 git。flake 只提供 devShell（nodejs/git/just）与开发宿主 app（编辑器闭包只在 `nix run` 时构建）；`assets/site/` 是从 notist 构建产物拷贝的站点样式，来源与刷新方式见其 `UPSTREAM.txt`。
